...
def load_cttm_facts():
    ...
    # 🚨 CRITICAL: PASTE THE PUBLISHED CSV URL HERE 🚨
    ...
    if CTTM_CSV_URL == "PASTE_YOUR_PUBLISHED_CSV_URL_HERE" or not CTTM_CSV_URL.endswith("output=csv"):
        st.warning("RAG Warning: CTTM Ledger URL not configured correctly. Using core Sīla principles only.")
        return ""
...
# =========================================================
# 3. GEMINI PROMPT CONSTRUCTION (Sīla, Samādhi, Paññā)
# =========================================================

def generate_system_prompt(cttm_facts):
    """Constructs the master prompt for the Gemini model."""

    # Note: Using f-string for Paññā injection
    system_prompt = f"""
    You are DHAMMI V7: The SS'ISM Constellation Advisor, powered by Google Gemini.
    Your identity is defined by the Sīla (Ethical Precepts), Samādhi (Focus/Power), and Paññā (Wisdom/Truth) framework designed by U Ingar Soe.

    ## Sīla (Ethical Precepts)
    ...
    --- CTTM LEDGER FACTS ---
    {cttm_facts if cttm_facts else "No verified facts currently loaded. Rely on core Sīla principles."}
    --- END FACTS ---

    ## Samādhi (Execution)
    1.  Respond with deep respect and use the greeting "🙏 Karunā (Compassion) and Greetings, U Ingar Soe."
    2.  Use Markdown formatting for clarity and bold key strategic concepts (Sīla, Samādhi, Paññā).
    3.  If a question relates to the Baydin operation, respond to the trigger "🔥🪄🔑" with the designated 5-page prediction output.
...
st.title("🛡️ DHAMMI V7: The SS'ISM Constellation Advisor")
st.caption("Powered by Gemini and Guided by Sīla, Samādhi, Paññā")
...
if prompt := st.chat_input("Ask DHAMMI V7 a question..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Meditating on Sīla, Samādhi, and Paññā..."):
